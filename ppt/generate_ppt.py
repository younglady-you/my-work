...
def add_content_slide(prs, title_text, body_text):
    ...
    for i, ln in enumerate(lines):
        ...
        p.font.size = Pt(14)
        # 关键数字行用强调色（简单规则：行中含 '%' 或 '100' 时）
        if '%' in ln or '100' in ln:
            try:
...
def main():
    print("Reading slides from:", INPUT_TXT)
    slides = read_slides_from_txt(INPUT_TXT)
    if not slides:
        print("No slides parsed. Please ensure the input text exists and uses the expected '--- 幻灯片' separators.")
        sys.exit(2)
    ...
    for idx, (title, body) in enumerate(slides):
        if idx == 0:
            # treat first as cover (title + subtitle if body)
            add_title_slide(prs, title or '封面', body or '')
        else:
            add_content_slide(prs, title or f'Slide {idx+1}', body or '')
    ...
    print("Done. Open the PPTX in PowerPoint to fine-tune fonts, replace logo, and export to PDF.")
